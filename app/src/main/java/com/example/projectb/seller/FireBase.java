...
public class FireBase {
    ...
    public void getS(String Ke){
        x.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for (DataSnapshot singleSnapshot : snapshot.child("Seller").getChildren()) {
                    try {
                        if(Ke.equals(singleSnapshot.getKey())) {
                            VALUE = singleSnapshot.child("product").getValue().toString() ;
                        }
                    }catch (Exception ex){ }
                }
            }
    ...
    public void getSellerAttribute(String KEY , String Atribute){
        x.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                VALUE = snapshot.child("Seller").child(KEY).child(Atribute).getValue().toString() ;
            }
    ...
    public String getSelleRAttribute(String KEY , String Atribute){
        ...
        x.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                val[0] = snapshot.child("Seller").child(KEY).child(Atribute).getValue().toString();
    ...
    public void DELETECHILD(String key , String sellerKey , String productKey){
        x.child("Product").child(key).removeValue();
        String s = "" ;
        for(int  i = 0  ; i < productKey.split(" ").length ; i++){
            if(!productKey.split(" ")[i].equals(key)){
                s = s + " " + productKey.split(" ")[i] ;
            }
        }
        x.child("Seller" ).child(sellerKey).child("product").setValue(s);
    }
    ...
    public void setSeller(String Name,String UName, String password, String phone, String location, String state) {
        ...
        x.child("Seller").child(key).child("location").setValue(location);
